...
```json
{
  ...
  "summary": "string — plain-language explanation of the verdict (rule-based when MOCK_LLM=true)"
}
```
...
```json
{
  ...
  "latency_ms": "integer",
  "drift_flag": "boolean",
  "summary": "string — fused Speed Layer explanation + latest Temporal Layer context"
}
```

> `rag_used: false` when the RAG service timed out and the verdict was resolved from Vision score alone.
> `summary` is generated by `_fuse_summary()` in the Aggregation Service, combining the per-frame RAG summary with the most recent `_latest_temporal` verdict for the same `stream_id`.
...
```json
{
  ...
  "timestamp_ms": "integer",
  "matched_signature": "string | null",
  "summary": "string — fused Speed + Temporal explanation for the Flagged Frames panel"
}
```
